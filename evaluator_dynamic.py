...
' - - - - - - - - VISUALIZATION - - - - - - - - - - - - '
def plot_tasks_animation(viz_frames, area):
    frames = []

    for frame in viz_frames:
        # --- Tasks ---
        tx = [t["x"] for t in frame["tasks"]]
        ty = [t["y"] for t in frame["tasks"]]
        trem = [t["remaining"] for t in frame["tasks"]]

        # --- Agents ---
        ax = [a["x"] for a in frame["agents"] if a["alive"]]
        ay = [a["y"] for a in frame["agents"] if a["alive"]]
        aid = [a["id"] for a in frame["agents"] if a["alive"]]
                ...
                data=[
                    ...
                    go.Scatter(
                        ...
                        marker=dict(
                            ...
                            colorscale="Viridis",
                            cmin=0,
                            cmax=max(trem) if trem else 1,
                            symbol="circle"
                        ),
                        name="Tasks",
                        text=[f"Task {t['id']}" for t in frame["tasks"]],
                    ),
                    # Agents
                    go.Scatter(
                        x=ax,
        ...
        data=frames[0].data,
        layout=go.Layout(
            xaxis=dict(range=[area[0], area[1]], title="X"),
            yaxis=dict(range=[area[2], area[3]], title="Y"),
            title="Dynamic Tasks + Agents",
            showlegend=True,
            updatemenus=[
                dict(
                    type="buttons",
...
def run(scn, agent_cls, log_path, trace_path=None):
    ...
    'PLOT'
    plot_tasks_animation(viz_frames, scn["area"])
